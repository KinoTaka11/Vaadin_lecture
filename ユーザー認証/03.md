...
Spring SecurityとVaadinを利用して認証機能を作成する。<br>
前項では`Spring-Security`を有効にし、それにより`Spring-Security`デフォルトのログインページが表示されることを確認した。
前項で述べたように、本項ではVaadin-Spring Security用の設定を行っていく。
...
public class SecurityConfig {
    ...
    // Vaadinの認証認可設定の自動構築と、それをSpring Securityに登録する@Bean
    @Bean
...
- `@Import(VaadinAware...)`は`Vaadin`と`Spring Security`の連携を正しく動作させるための設定を取り込むアノテーションである。

- `securityFilterChain()`はVaadinが構築した認証認可の設定しており、`@Bean`をつけることにより`Spring Security`に構築された設定を渡している。
