...
Generateボタンを押して、Vaadin_prac.zipファイルをダウンロードする。
ダウンロードした Vaadin_prac.zip は展開する。
...
書き換え前（ `<version>...</version>` の値は、プロジェクトを作成した時に選んだ `3.3.x` になっているはず）
...
<parent>
  ...
  <relativePath>pom-parent/spring-boot-33.xml</relativePath>
</parent>
```
